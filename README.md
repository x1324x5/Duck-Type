...
**一块仪表盘，而不是一张报表。** 首屏先给你最关心的几个数字——总字数、平均与峰值速度、活跃时长、修改率——再往下是今日目标环、每日产出曲线、按小时的输入节奏，以及高频字、高频词、词性分布和一张「星期 × 小时」的活跃热力图。点任意一个字或词，都能跳进去看它的来龙去脉。0.3.0 起还多了三块洞察：**词汇量成长**（你累计写过的不同词 / 字随时间一路变大的曲线）、**各应用效率**（你在哪个软件里打得最快）、**工作日 vs 周末**（你的产出节奏更偏平时还是周末）。看板下半部分的面板会在滚动到时才加载，首屏更快。
...
DuckType 目前是**未签名**的程序，所以第一次运行时 Windows SmartScreen 很可能弹出「Windows 已保护你的电脑」蓝色窗口。这不代表它有病毒，只是因为这个 `.exe` 还没有购买代码签名证书、也还没积累足够的下载声誉。放行方法：

1. 在弹窗里点左下角的「**更多信息 / More info**」。
2. 再点「**仍要运行 / Run anyway**」即可。

如果你想再确认一下文件没被篡改，每个 [Release](https://github.com/x1324x5/Duck-Type/releases) 都会附上 `DuckType.exe` 的 **SHA-256** 校验值，可在 PowerShell 里用
`Get-FileHash .\DuckType.exe -Algorithm SHA256` 比对。只从本仓库的 Releases 页下载，不要从第三方站点拿。

> 想彻底不再弹这个提示，需要给 exe 加代码签名——详见 [开发与测试](#开发与测试) 末尾「关于代码签名」。
...
### 关于代码签名（消除 SmartScreen 提示）

未签名的 exe 在用户机器上会触发 SmartScreen 警告（见上文「第一次打开被 Windows 拦住了？」）。要彻底消除它，需要给打包产物加 **Authenticode 代码签名**，可选路径，按性价比从高到低：

- **EV 代码签名证书**（约 ¥2,000–3,000/年，需硬件 token 或云 HSM）：签名后基本能**立刻**获得 SmartScreen 信誉，新版本不再弹警告。对要持续分发的项目最省心。
- **OV / 标准代码签名证书**（约 ¥1,000–1,500/年）：能去掉「未知发布者」，但 SmartScreen 信誉仍需靠下载量**慢慢积累**，初期可能还会弹一阵。
- **暂不签名**：保持现状，在发布说明里写清放行步骤 + 附 SHA-256 校验值（CI 已可在打包后输出）。这是目前采用的方案。

签名命令示意（拿到证书后，在 PyInstaller 产物上执行一次即可）：

```bat
signtool sign /fd SHA256 /tr http://timestamp.digicert.com /td SHA256 dist\DuckType.exe
```

证书私钥**绝不要**提交进仓库或 CI 明文环境变量；用 GitHub Actions 的加密 secret + 云签名服务（如 Azure Trusted Signing / SignPath 的开源计划）最稳妥。SignPath 对符合条件的开源项目提供免费签名，是预算有限时的首选。
